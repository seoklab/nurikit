...
public:
  ...
  static const absl::flat_hash_map<std::string_view, AminoAcid> &data();
...
const absl::flat_hash_map<std::string_view, AminoAcid> &AminoAcid::data() {
  // clang-format off
  static const absl::flat_hash_map<std::string_view, AminoAcid> the_data {
    { "ALA", {
      {
        { "N", 7, 2 },
...
void update_std_atoms(Molecule &mol, const std::vector<Substructure> &subs) {
  const auto &aa_data = AminoAcid::data();
