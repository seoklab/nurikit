    ...
    for (int root = 0; root < mol.num_atoms(); ++root) {
      ...
      do {
        ...
        for (auto nei: mol[curr]) {
          ...
          int next_ti;
          if (nei.edge_data().is_ring_bond() || nei.src().degree() == 1
              || nei.dst().degree() == 1) {
            next_ti = curr_ti;
...
namespace internal {
  std::vector<GARotationInfo> ga_resolve_rotation_info(const Molecule &mol) {
