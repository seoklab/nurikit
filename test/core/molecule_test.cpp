...
TEST_F(MoleculeTest, TransformTest) {
...

  {
    ...
    mol.transform(trs);
    for (int i = 0; i < mol.num_conf(); ++i) {
      EXPECT_TRUE(mol.conf(i).isApprox(trs * mol_.conf(i)));
    }
  }

  {
    ...
    EXPECT_TRUE(mol.conf(1).isApprox(trs * mol_.conf(1)));
  }
}
...
TEST_F(MoleculeTest, RotateBondTest) {
  ...
  for (int i = 0; i < mol_all.num_conf(); ++i) {
    EXPECT_TRUE(mol_all.conf(i)(Eigen::all, fixed)
                    .isApprox(mol_.conf(i)(Eigen::all, fixed)));
  }
  ...
  EXPECT_TRUE(
      mol_all.conf(0)(Eigen::all, rotated).isApprox(result_coords, 1e-3))
      << mol_all.conf(0)(Eigen::all, rotated);
  ...
  EXPECT_TRUE(
      mol_all.conf(1)(Eigen::all, rotated).isApprox(result_coords, 1e-3))
      << mol_all.conf(1)(Eigen::all, rotated);
...
TEST_F(MoleculeTest, EraseAtomsTest) {
  ...
  for (int i = 0; i < mol1.num_conf(); ++i) {
    EXPECT_TRUE(mol1.conf(i).isApprox(mol_.conf(i).leftCols<9>()));
    EXPECT_EQ(mol1.conf(i).cols(), 9);
  }
  ...
  for (int i = 0; i < mol2.num_conf(); ++i) {
    EXPECT_TRUE(mol2.conf(i).isApprox(mol_.conf(i).rightCols<9>()));
    EXPECT_EQ(mol2.conf(i).cols(), 9);
  }
  ...
  for (int i = 0; i < mol3.num_conf(); ++i) {
    EXPECT_TRUE(mol3.conf(i).isApprox(mol_.conf(i)(Eigen::all, keep)));
    EXPECT_EQ(mol3.conf(i).cols(), 9);
  }
}
